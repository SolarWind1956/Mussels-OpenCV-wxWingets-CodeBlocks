 ...
 *************************************************************
*/
#include "erodeTestFrame.h"
/*
    Класс тестирования преобрзования `erodeTestFrame`
*/
//	-----------------------------------------------------------------------------------------------------
/*
...
*/
erodeTestFrame::erodeTestFrame  ( wxWindow  *   parent, const wxString& title)
                        : wxFrame   (   parent
                                    ,   wxID_ANY
                                    ,   title
                                    ,   wxDefaultPosition
                                    ...
                                    )
{
    ...
    m_transform_name_text       =
            new wxStaticText(m_transform_namePanel, -1, wxT("erode morphological transformations"), wxPoint(0, 0));
    ...
    m_zoom_slider_ctrl->Bind(wxEVT_SLIDER, &erodeTestFrame::OnZoomSliderChanged, this);
    m_kernel_width_ctrl->Bind(EVT_KERNEL_CHANGED, &erodeTestFrame::OnKernelWidthChanged, this);
    m_kernel_height_ctrl->Bind(EVT_KERNEL_CHANGED, &erodeTestFrame::OnKernelHeightChanged, this);
    m_kernel_shape_radiobox_ctrl->Bind(wxEVT_RADIOBOX, &erodeTestFrame::OnKernelShapeChanged, this);
    m_iterations_spin_ctrl->Bind(wxEVT_SPINCTRL, &erodeTestFrame::OnIterationsChanged, this);
    m_border_chois_ctrl->Bind(wxEVT_CHOICE, &erodeTestFrame::OnBorderExtrapolChanged, this);
...
void erodeTestFrame::OnResize(wxSizeEvent& event)
{
...
void erodeTestFrame::OnZoomSliderChanged(wxCommandEvent& event) {
    m_zoom_slider_ctrl->m_zoom = event.GetInt();
    UpdateAllViews();
}

void erodeTestFrame::OnKernelWidthChanged(wxCommandEvent& event) {
...
void erodeTestFrame::OnKernelHeightChanged(wxCommandEvent& event) {
    // 1. Извлекаем размер ядра, который мы положили в SetInt() в контроле
    int newSize = event.GetInt();
...
void erodeTestFrame::OnKernelShapeChanged(wxCommandEvent& event) {
...
void erodeTestFrame::OnIterationsChanged(wxCommandEvent& event) {
...
void erodeTestFrame::OnBorderExtrapolChanged(wxCommandEvent& event) {
...
void erodeTestFrame::UpdateAllViews() {
...
//  -----------------------------------------------------------------------------------------------------
//  Экосистема OpenCV
void erodeTestFrame::ApplyMorphologicalTransformations() {
    // Вся "кухня" OpenCV здесь
    m_kernel_shape = GetSelectedMorphShape();
    cv::erode   (   m_cv_original_img
                ,   m_cv_filtered_img
...
std::string erodeTestFrame::getSignatureText(){
...
wxArrayString   erodeTestFrame::getKernelShape(){
...
int erodeTestFrame::GetSelectedMorphShape() {
